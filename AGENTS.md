...
- 자동 추천에서 크롤링 주문 등록과 입금 추천 완료 처리는 `import_crawl_order`, `complete_deposit_recommendation` PostgreSQL RPC로 관련 행 변경을 한 트랜잭션에서 처리해 중간 실패·동시 처리를 안전하게 막습니다.
...
- 구매장부(`/`)는 `orders.is_order_completed`(주문완료)와 `orders.is_processed`(입금완료)를 분리해 관리합니다. 기존 완료의 의미는 입금완료로 유지하며, 주문완료는 미입금 주문 목록의 노란색 `주문완료처리` 버튼으로 즉시 저장합니다. 데스크톱에서는 입금완료처리 버튼 위에, 모바일에서는 펼침 패널의 주문완료처리 아래에 입금완료처리 버튼을 쌓아 표시하며, 입금완료처리는 토스트형 입력창에서 처리합니다. 주문상세보기 버튼은 그 아래에 표시하며, 주문완료 주문은 노란색으로 강조합니다. 주문완료 건수는 두 상태를 합산하고, `status=order_completed` 필터로 주문완료·입금완료 주문을 함께 봅니다. 상단 `사용 방법` 버튼에서 주문완료·입금완료의 상태 흐름과 처리 위치를 언제든 확인할 수 있으며, 구매장부 첫 접속 시 계정·버전별로 한 번 자동 표시합니다. 입금완료처리는 모바일 토스트형 입력창과 데스크톱 드롭다운에서 입금일자를 오늘(한국 시간), 입금금액을 구매금액, 입금메모를 카톡방 이름(`orders.title`)으로 기본 입력합니다. 입금일자는 `-1일`/`+1일`, 입금금액은 `-500원`/`+500원` 버튼으로 보정할 수 있습니다. 완료 직전 배송 없는 상품의 구매금액과 입금금액이 같거나, 배송 있는 상품의 두 금액이 다르면 커스텀 확인창에서 `취소하기` 또는 `무시하고 처리하기`를 선택하게 하며 Enter 키는 확인으로 처리합니다.
...
- 구매장부(`/`)는 서버에서 인증 사용자·`orders` count·미완료 주문 초기 조회를 병렬 preload한 뒤 클라이언트 SWR 초기값으로 사용합니다. 입금완료 주문은 기본 접힘 상태이며 사용자가 입금완료 주문 섹션을 펼칠 때 `is_processed = true` 행을 처음 조회하고 이후 캐시합니다. 미완료·입금완료 목록은 화면 내부 검색 기준으로 동작하는 가상 스크롤을 사용하며, 화면 크기에 따라 모바일 카드 또는 데스크톱 테이블 중 하나만 마운트해 이중 렌더링을 피합니다.
- 구매장부의 미완료·입금완료 주문 헤더·목록 UI는 각각 top-level memoized 컴포넌트가 담당하고, `OrdersTable`은 두 목록의 조회·필터·가상 스크롤 계산·mutation 상태를 유지합니다. 데스크톱 표와 모바일 카드 중 현재 화면에 맞는 branch만 마운트하는 동작은 그대로 유지합니다.
...
- 구매장부에는 미완료 원금·예약 확인·완료 정보 확인을 묶은 오늘의 운영 요약과 빠른 작업 링크를 표시하며, 실 배송 여부는 별도 장부 필터와 통계 정보로 표시합니다. 모바일에서는 오늘의 운영 요약을 우측 상단 알람 버튼으로 열고 빠른 작업을 우측 하단 + 버튼의 팝업으로 열며, 데스크톱에서는 기존 보조 패널로 표시합니다. 대시보드는 기간 요약·전월 비교·월별/완료율/분류별 Recharts를 제공하고, 월별 상세는 현재 월과 전월을 함께 조회해 비교합니다.
...
- 전체 검색은 데스크톱 사이드바 또는 모바일 페이지 헤더에서 열 수 있고 `Ctrl/Cmd + K`도 지원합니다. 메뉴·설정 하위 화면·주문 제목/상품/주문번호/메모·템플릿 제목을 검색하며, 주문 결과는 상태·배송 여부·구매일·금액을 표시하고 전체 결과는 같은 검색어가 적용된 장부로 이어집니다.
...
- 대시보드와 월별 상세의 핵심 수치·운영 인사이트·분류 행은 해당 기간과 조건이 적용된 구매장부로 이동합니다. 대시보드는 현재 선택 기간만 별도 엑셀로 내보낼 수 있고, 월별 상세는 큰 금액 KPI 4개와 주문 상태·배송 여부·수익 효율 그룹으로 정보 우선순위를 나눕니다.
...
- Google Play 배포에 사용하는 공개 개인정보처리방침은 `/privacy`, 계정·연결 데이터 삭제 요청 안내는 `/account-deletion`에서 로그인 없이 확인할 수 있습니다. 로그인·정책·삭제 안내 화면에는 인증 후 업무용 사이드바와 하단 메뉴를 표시하지 않습니다. 로그인 화면과 설정의 계정 화면에서 두 문서로 이동하며, 계정은 앱에서 직접 만들지 않고 운영자가 발급합니다. 삭제 요청과 개인정보 문의는 공개 지원 이메일 `jinitlab@gmail.com`으로 접수합니다.
...
- 2차 2B 대량 처리 기능으로 구매장부에 데스크톱 체크박스·모바일 명시적 선택 모드와 고정 작업 막대를 추가했습니다. 선택 주문은 실 배송 여부·플랫폼·결제수단·구매계정·구매 정보 템플릿을 변경 전/후 확인 뒤 일괄 수정할 수 있고, 선택 범위만 엑셀로 내보낼 수 있습니다. 일부 주문만 실패하면 성공 건은 반영하고 실패 건만 선택 상태로 남깁니다.
- 일괄 완료처리는 단건 완료처리와 같은 기본 입금값·배송 여부/금액 경고·수익 계산을 공통 로직으로 사용합니다. 주문별 입금일·입금금액·메모를 수정한 뒤 요약 확인과 최종 확인을 거치는 단계형 화면으로 처리합니다.
