...
- 구매장부(`/`)는 `orders.is_order_completed`(주문완료)와 `orders.is_processed`(입금완료)를 분리해 관리합니다. 기존 완료의 의미는 입금완료로 유지하며, 주문완료는 미입금 주문 목록의 노란색 `주문완료처리` 버튼으로 즉시 저장합니다. 데스크톱에서는 입금완료처리 버튼 위에, 모바일에서는 펼침 패널의 주문상세보기 위에 표시하며, 주문완료 주문은 노란색으로 강조합니다. 주문완료 건수는 두 상태를 합산하고, `status=order_completed` 필터로 주문완료·입금완료 주문을 함께 봅니다. 상단 `사용 방법` 버튼에서 주문완료·입금완료의 상태 흐름과 처리 위치를 언제든 확인할 수 있으며, 구매장부 첫 접속 시 계정·버전별로 한 번 자동 표시합니다. 입금완료처리는 모바일 펼침 패널과 데스크톱 드롭다운에서 입금일자를 오늘(한국 시간), 입금금액을 구매금액, 입금메모를 카톡방 이름(`orders.title`)으로 기본 입력합니다. 입금일자는 `-1일`/`+1일`, 입금금액은 `-500원`/`+500원` 버튼으로 보정할 수 있습니다. 완료 직전 배송 없는 상품의 구매금액과 입금금액이 같거나, 배송 있는 상품의 두 금액이 다르면 커스텀 확인창에서 `취소하기` 또는 `무시하고 처리하기`를 선택하게 하며 Enter 키는 확인으로 처리합니다.
