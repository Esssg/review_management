...
- 구매장부의 완료 주문 헤더·목록 UI는 top-level memoized 컴포넌트가 담당하고, `OrdersTable`은 완료 주문 조회·필터·가상 스크롤 계산·mutation 상태를 유지합니다. 데스크톱 표와 모바일 카드 중 현재 화면에 맞는 branch만 마운트하는 동작은 그대로 유지합니다.
