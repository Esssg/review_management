...
- 주문 상세(`/orders/detail`)는 서버에서 주문·master data·구매 정보 템플릿을 초기 preload하고, 브라우저에서는 visibility 시 `ai_review` 최신값만 다시 확인합니다. 폼의 저장·dirty guard·AI 동작은 기존 클라이언트 책임을 유지합니다.
