...
- 설정 홈에는 리뷰 매니저 브랜드 아이콘을 사용하는 PWA 설치 카드를 표시합니다. Chromium 계열은 설치 프롬프트를 열고, iOS Safari 등 프롬프트를 제공하지 않는 환경은 브라우저별 홈 화면 추가 안내를 보여줍니다. 앱 사용 권한 설정에서는 Web Push 권한과 기기별 구독을 관리합니다. 구매 예정 시각 알림은 서비스 워커와 서버 예약 작업으로 발송하고, 앱 알림 내역은 미확인 상태만 표시합니다. `vm-app-01`에서는 `pg_cron` 스키마가 없어 `/opt/supabase/docker/scripts/process-purchase-notifications.sh`를 사용자 cron으로 매분 실행합니다. 주문·Supabase 데이터는 오프라인 캐시하지 않습니다.
