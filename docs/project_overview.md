...
- 자동 추천(`/menu-4`)은 현재 보고 있는 추천 페이지만 마운트합니다. 입금 내역 자동 추천에 필요한 입금 계좌·미완료 입금·주문 후보 데이터는 입금 추천 페이지에 진입할 때 조회하고, 펼친 입금 건의 추천 결과는 주문 후보 버전 기준으로 캐시해 같은 유사도 계산을 반복하지 않습니다.
