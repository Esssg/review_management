...
Supabase/PostgREST의 기본 반환 제한(현재 1,000건)은 최종 데이터 건수가 아니라 한 번에 가져오는 페이지 크기입니다.

- 목록·검색·통계·추천 등 전체 행이 필요한 조회는 `range(from, to)` 페이지네이션 또는 동등한 방식으로 이후 페이지를 끝까지 가져옵니다.
- 페이지를 나눠 가져올 때는 안정적인 정렬 기준을 사용해 페이지 사이의 누락·중복을 방지합니다.
- 첫 1,000건만 조회하거나 `.limit(1000)`을 전체 결과로 사용하지 않습니다. 최신순 상위 N건처럼 일부 행만 필요한 경우에만 제한 조회를 사용하고, 그 의도를 코드에 드러냅니다.
